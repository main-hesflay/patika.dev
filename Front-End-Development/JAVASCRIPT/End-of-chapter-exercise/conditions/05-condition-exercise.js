// Kosullarla Calismak Bolum Egersizi:
...
function score() { // bunları fonksiyon içerisine aldım yani butona tıkladığı zaman aşağıdaki kod bloğu çalışıcak.
...
`
    ...
    if (examGrade >= 0 && examGrade <= 100) {
        // tüm if yapilari buraya gelsin

        textInfo = SMILE // smile icon
        info.classList.add('text-primary') // text-primary diyerek renklerini mavi yaptık 
        ...
        if (examGrade >= 90) {
            ...
            textInfo = `${SAD} FF` // ancak ff notunda ise sadece sad icon koyduk

            info.classList.remove('text-primary')
            info.classList.add('text-danger') // burada sadece ff notu alan text-danger yani kırmızı rengini yaptık
        }
    } else {
